...
import { LayoutDashboard, FileText, LogOut, User, Upload, Camera, Moon, Sun, Settings, HelpCircle } from "lucide-react";
...
export function IssuerLayout({ children, currentPage, onNavigate }: IssuerLayoutProps) {
  ...
  const menuItems = [
    ...
    { id: "create-bond", icon: FileText, label: "Launch Investment Opportunity" },
    { id: "milestones", icon: Upload, label: "Submit Progress Proof" },
    { id: "project-updates", icon: Camera, label: "Project Updates" },
  ];
        ...
        <div className="px-6 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-semibold uppercase tracking-wider px-2.5 py-1 bg-emerald-100 text-emerald-700 dark:bg-emerald-900/50 dark:text-emerald-300 rounded-full">
                Project Developer
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Notifications */}
            <NotificationDropdown role="issuer" />

            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-emerald-400 to-green-600 flex items-center justify-center text-white text-xs font-bold shadow-sm">
              {userInitials}
            </div>
        ...
        <aside className="w-64 sidebar-gradient border-r border-border/50 min-h-[calc(100vh-57px)] sticky top-[57px] flex flex-col h-[calc(100vh-57px)]">
            ...
            {menuItems.map((item, idx) => (
              <button
                ...
                className={cn(
                  ...
                  currentPage === item.id
                    ? "bg-gradient-to-r from-emerald-600 to-green-700 text-white shadow-md shadow-emerald-500/20 dark:from-emerald-500 dark:to-green-600"
          ...
          <div className="mt-auto p-3 border-t border-border/50 bg-background/30">
            <div className="flex items-center gap-2.5 px-2.5 py-2 mb-2 rounded-lg">
              <div className="w-9 h-9 rounded-full bg-gradient-to-br from-emerald-400 to-green-600 flex items-center justify-center text-white text-xs font-bold shadow-sm">
                {userInitials}
              </div>
              <div className="min-w-0">
                <p className="text-sm font-semibold text-foreground leading-tight truncate">{user?.name || "Project Developer"}</p>
                <p className="text-[11px] text-muted-foreground leading-tight truncate">{user?.email || "issuer@infrabondx.com"}</p>
              </div>
            </div>
